    ...
    <div style={{ backgroundColor: bg, color: 'white', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', WebkitFontSmoothing: 'antialiased' }}>
      <nav style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '24px 0', position: 'sticky', top: 0, backgroundColor: 'rgba(8,8,8,0.95)', backdropFilter: 'blur(10px)', zIndex: 100 }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setView('booking')} style={{ backgroundColor: accent, color: 'black', border: 'none', padding: '10px 24px', borderRadius: '2px', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', cursor: 'pointer', letterSpacing: '2px' }}>
      ...
      {view === 'home' ? (
        <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          <section style={{ display: 'flex', flexWrap: 'wrap', gap: '60px', alignItems: 'center', padding: '100px 0' }}>
            <div style={{ flex: '1 1 450px' }}>
              ...
              <h2 style={{ fontSize: 'clamp(3.5rem, 10vw, 8.5rem)', fontWeight: '900', textTransform: 'uppercase', margin: '0 0 32px 0', lineHeight: '0.8', letterSpacing: '-5px' }}>Force <br /> <span style={{ color: accent }}>Mastered.</span></h2>
              <p style={{ color: '#888', fontSize: '18px', maxWidth: '480px', lineHeight: '1.6', fontWeight: '400', marginBottom: '40px' }}>Bridging the gap between clinical science and raw power. We utilize advanced biomechanical data to engineer a swing that is faster, more efficient, and pain-free.</p>
              <button onClick={() => setView('booking')} style={{ backgroundColor: 'transparent', color: accent, border: `1px solid ${accent}`, padding: '16px 32px', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '3px', cursor: 'pointer' }}>Initialize Assessment</button>
            </div>
            <div style={{ flex: '1 1 400px' }}>
              <img src="https://images.unsplash.com/photo-1593111774240-d529f12cf4bb?q=80&w=2076&auto=format&fit=crop" alt="Golf Athlete" style={{ width: '100%', height: '600px', objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)', filter: 'grayscale(1) brightness(0.7)' }} />
            </div>
          </section>
        </main>
      ) : (
        <main style={{ maxWidth: '800px', margin: '0 auto', padding: '100px 24px' }}>
          ...
          <form style={{ backgroundColor: 'rgba(255,255,255,0.02)', padding: '40px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <button type="button" style={{ width: '100%', backgroundColor: accent, color: 'black', border: 'none', padding: '20px', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '4px', cursor: 'pointer' }}>Submit Intake For Review</button>
          </form>
        </main>
      )}
